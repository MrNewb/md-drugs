...
local function createLabKit(coord, head)
    ...
    AddMultiModel(heroinlabkit, {
        ...
        {data = heroinlabkit, event = "md-drugs:client:cleanheroinlabkit",icon = "fa-solid fa-hand-sparkles",label = locale("targets.heroin.clean"),canInteract = function() if dirty then return true end end}},heroinlabkit)
end

local function pickher(loc)
    if not progressbar(locale("Heroin.pick"), 4000, 'uncuff') then return end
...
RegisterNetEvent('heroin:respawnCane', function(loc)
    ...
    if not PoppyPlants[loc] then
        ...
        AddSingleModel(PoppyPlants[loc], {icon = "fa-solid fa-seedling", label = locale("targets.heroin.pick"), action = function() pickher(loc) end}, loc )
    end
end)
...
RegisterNetEvent("heroin:init", function()
    for k, v in pairs (GlobalState.PoppyPlants) do
        ...
        if not v.taken then
            ...
            AddSingleModel(PoppyPlants[k], {icon = "fa-solid fa-seedling", label = locale("targets.heroin.pick"), action = function() pickher(k) end}, k )
        end
    end
end)
...
RegisterNetEvent("md-drugs:client:dryplant", function(data) 
    if not progressbar(locale("Heroin.dryout"), 4000, 'uncuff') then return end
...
RegisterNetEvent("md-drugs:client:heatliquidheroin", function(data) 
	...
	return end
    if not progressbar(locale("Heroin.success"), 4000, 'uncuff') then return end
...
RegisterNetEvent("md-drugs:client:getheroinkitback", function(data) 
   if not progressbar(locale("Heroin.pickup"), 4000, 'uncuff') then return end
    herointable = false
